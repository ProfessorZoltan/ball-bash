 ...
 * or {at, toward}, aimed at a mark. The Far Course's pieces fly as they do in
 * the game: glass breaks, a switch opens its doors, and the emitters' rings shove.
 */
async function golfFly(def, angle, { pulses = [], maxT = null, events = false, watch = null, launchAt = 0, path = false } = {}) {
  let warps = 0;
  ...
  // A switch opens its doors for a while and glass fast enough breaks, as main.js's onWallBounce has it.
  const onWall = (h, seg, before) => {
    bounces++;
    if (seg.kind === 'node' && seg.node.kind === 'switch') golfSwitch(g, seg.node, g.mouthTime);
    else if (paneBreaks(g, seg, before)) breakPane(g, seg.pane, before, g.mouthTime);
  };
  ...
  const pts = path ? [] : null; // where the charge was each step, when asked
  for (; t < (maxT || def.flightSeconds); t += PHYSICS_DT) {
    ...
    for (const p of plan) {
      ...
      p.done = true;
      // A pulse is spent along a heading, or aimed at a mark ('cup', or [x, y]) the way a player points the heading at one.
      const mark = p.toward === 'cup' ? [cup.x, cup.y] : p.toward;
      const a = mark ? Math.atan2(mark[1] - b.y, mark[0] - b.x) : p.a;
    ...
    advanceBall(b, g.walls, [], PHYSICS_DT, SURFACE_VELOCITY_FACTOR, { onWall, onMover: () => bounces++ }, movers, solidPolysNow(g));
    b.clampSpeed(BALL.minSpeed, g.maxSpeed);
...
test('golf: a hole is a level with no opponent - a launcher bolted to the tee, no boss, no drones, and its cup among its bodies', async () => {
  const { createGameState } = await import('../src/gamestate.js');
  const { COURSE, COURSE_PAR, FAR_COURSE, FAR_COURSE_PAR, COURSES, holeLabel, toPar } = await import('../src/golf.js');
  assert.ok(COURSE.length >= 3, 'the course has holes on it');
  assert.equal(COURSE_PAR, COURSE.reduce((n, h) => n + h.par, 0));
  assert.equal(FAR_COURSE_PAR, FAR_COURSE.reduce((n, h) => n + h.par, 0));
  assert.equal(COURSES.outer.holes, COURSE);
  assert.equal(COURSES.far.holes, FAR_COURSE);
  const ids = COURSE.concat(FAR_COURSE).map((h) => h.id);
  assert.equal(new Set(ids).size, ids.length, 'every hole on both courses has its own id, so the bests never mix');
  FAR_COURSE.forEach((h, i) => {
    assert.equal(h.hole, i + 1, `${h.title} is numbered in order`);
    assert.equal(h.course, 'far');
  });
  assert.equal(toPar(0), 'level');
  assert.equal(toPar(2), '+2');
  assert.equal(toPar(-1), '-1');
  for (const def of COURSE.concat(FAR_COURSE)) {
    const g = createGameState(def, {});
    ...
    assert.ok(g.nodes.every((n) => n.kind === 'switch') && g.turrets.length === 0, `${def.title}: a hole carries no targets (a switch is a door's handle, not a target)`);
    ...
    // A hole with a dry gauge is one launch and nothing after it, and may cut its clock short; the rest leave time to steer.
    assert.ok(def.flightSeconds >= (def.fuel === 0 ? 3.5 : 5), `${def.title}: a flight clock the hole can be played in`);
...
test('golf, the last four: every one bigger than a screen, built from pieces the course already taught, no two built alike, and the last the hardest', async () => {
  const { createGameState } = await import('../src/gamestate.js');
  const { COURSE } = await import('../src/golf.js');
  const last = COURSE.slice(14);
  assert.deepEqual(last.map((h) => h.hole), [15, 16, 17, 18]);
  // The pieces a hole is built from, named as the course introduced them.
  const pieces = (def) => {
    const g = createGameState(def, {});
    const k = new Set();
    for (const w of g.wells) {
      if (w.cup) continue;
      if (w.fount) k.add('fount');
      else if (w.solid && w.pull >= 150000) k.add('orbit body');
      else if (w.solid) k.add('stone');
      else if (w.rail) k.add('maw on a rail');
      else k.add('maw');
    }
    for (const w of g.wormholes) {
      k.add(w.orbitA || w.orbitB ? 'moving mouth' : 'mouth');
      if (w.oneWay) k.add('one-way mouth');
      if (w.color) k.add('coloured pairs');
    }
    for (const m of g.movers) if (m.kind !== 'stone') k.add(m.kind === 'orbiter' ? 'cage' : 'turning bar');
    if (def.open) k.add('open space');
    if (g.maxSpeed < 500) k.add('heavy charge');
    return k;
  };
  const sets = last.map(pieces);
  last.forEach((def, i) => {
    assert.ok(def.view && (def.width > def.view.w || def.height > def.view.h), `${def.title} is bigger than one screen`);
    assert.ok(sets[i].size >= 3, `${def.title} mixes pieces: ${[...sets[i]].join(', ')}`);
  });
  for (let i = 0; i < 4; i++) for (let j = i + 1; j < 4; j++) {
    const same = [...sets[i]].filter((x) => sets[j].has(x)).length;
    assert.ok(same < Math.min(sets[i].size, sets[j].size), `${last[i].title} and ${last[j].title} are built from the same pieces`);
  }
  const final = last[3];
  assert.ok(COURSE.every((h) => h === final || h.par < final.par), 'the last hole has the highest par on the course');
  assert.ok(sets[3].size >= Math.max(...sets.slice(0, 3).map((x) => x.size)), 'and more pieces than any of the other three');
  assert.ok(final.noBareLine, 'and nothing sinks it without the gauge');
});

test('golf: Lenses - two founts make a lens that sends every line to one point, and the way round it is along a maw\'s edge', async () => {
  const { COURSE } = await import('../src/golf.js');
  const def = COURSE.find((h) => h.id === 'g15');
  const rad = (d) => (d * Math.PI) / 180;
  assert.ok(def.open && def.obstacles.length === 0, 'open space: nothing to bank off');
  // A lens: lines through the first pair spread 60 px apart at the founts meet within a few px just beyond them, then fan out again.
  const yAt = (pts, x) => pts.find((p) => p[0] >= x)[1];
  const through = [];
  for (const d of [-20, -18, -16, -14, -12]) through.push((await golfFly(def, rad(d), { maxT: 4, path: true })).pts);
  const spread = (x) => Math.max(...through.map((p) => yAt(p, x))) - Math.min(...through.map((p) => yAt(p, x)));
  assert.ok(spread(9950) > 50, `spread at the lens: ${spread(9950).toFixed(0)}`);
  assert.ok(spread(10100) < 12, `and at its focus: ${spread(10100).toFixed(0)}`);
  assert.ok(spread(10300) > spread(10100) * 5, 'then apart again');
  // The tee points through the lens, and that line touches nothing and goes nowhere.
  const opener = await golfFly(def, def.tee.angle);
  assert.equal(opener.end, 'spent');
  assert.equal(opener.bounces, 0);
  // The way in: under the lens and along the maw's edge, a few px outside its horizon, then round the stone and through the second lens.
  const maw = def.wells.find((w) => w.hazard && w.x < 11000);
  const line = await golfFly(def, rad(13), { path: true });
  assert.equal(line.end, 'cup', `13 degrees goes down (${line.end})`);
  const edge = Math.min(...line.pts.map((p) => Math.hypot(p[0] - maw.x, p[1] - maw.y)));
  assert.ok(edge > maw.r && edge < maw.r + 20, `past the maw ${Math.round(edge - maw.r)} px outside its horizon`);
  const noMaw = { ...def, wells: def.wells.filter((w) => w !== maw) };
  assert.notEqual((await golfFly(noMaw, rad(13))).end, 'cup', 'without the maw to turn it, the same line misses');
  // A narrow band: a few degrees at most of the whole circle.
  let band = 0;
  for (let d = 0; d < 40; d += 0.5) if ((await golfFly(def, rad(d))).end === 'cup') band++;
  assert.ok(band >= 2 && band <= 8, `${band / 2} degrees sink it`);
});

test('golf: Rendezvous - park, climb to meet a mouth on a wider orbit, past a maw that rides a rail the other way', async () => {
  const { createGameState, placeRails } = await import('../src/gamestate.js');
  const { COURSE } = await import('../src/golf.js');
  const def = COURSE.find((h) => h.id === 'g16');
  const rad = (d) => (d * Math.PI) / 180;
  const g = createGameState(def, {});
  const body = g.wells.find((w) => w.solid);
  const moon = g.wells.find((w) => w.rail);
  const [w] = g.wormholes;
  const park = Math.hypot(def.tee.x - body.x, def.tee.y - body.y);
  assert.ok(park < moon.rail.R - moon.range && moon.rail.R + moon.range < w.orbitA.R - w.r, 'the maw\'s rail lies between the parking orbit and the mouth\'s, touching neither');
  assert.ok(Math.sign(moon.rail.period) !== Math.sign(w.orbitA.period), 'and turns the other way');
  assert.ok(w.oneWay, 'the mouth only lets go at its far end');
  // A maw on a rail moves on the level's clock: half a period on, it is on the far side.
  const at0 = [moon.x, moon.y];
  placeRails(g.wells, Math.abs(moon.rail.period) / 2);
  assert.ok(Math.hypot(moon.x - (2 * moon.rail.cx - at0[0]), moon.y - (2 * moon.rail.cy - at0[1])) < 1e-6, 'half a period on, it is opposite');
  // The tee's line parks: round the body for the whole clock, touching nothing.
  const parked = await golfFly(def, def.tee.angle);
  assert.equal(parked.end, 'spent');
  assert.equal(parked.warps + parked.bounces, 0);
  // The plan: launched 4.2 s into the hole, one burn along the flight two laps in, through the mouth at the top of the climb, and one pulse to steer in.
  const plan = [{ at: 7.76, a: rad(233) }, { at: 9.34, a: rad(0) }];
  const met = await golfFly(def, def.tee.angle, { launchAt: 4.2, pulses: plan });
  assert.equal(met.end, 'cup', `the rendezvous goes down (${met.end})`);
  assert.equal(met.warps, 1);
  // The same plan at the other moments of a whole turn of the mouth: it is almost never there, and the maw on the rail often is.
  let also = 0;
  let intoMaw = 0;
  let n = 0;
  for (let T = 4.7; T < 4.2 + Math.abs(w.orbitA.period); T += 0.1) {
    const r = await golfFly(def, def.tee.angle, { launchAt: T, pulses: plan });
    n++;
    if (r.end === 'cup') also++;
    if (r.end === 'maw') intoMaw++;
  }
  assert.ok(also <= 3, `the same plan also goes down at ${also} of ${n} other moments`);
  assert.ok(intoMaw >= 10, `and goes into the maw on the rail at ${intoMaw} of them`);
});

test('golf: Heavy Water - three screens down on a heavy charge, two pairs in order, a stone to turn and a cage round the second mouth', async () => {
  const { createGameState } = await import('../src/gamestate.js');
  const { COURSE } = await import('../src/golf.js');
  const def = COURSE.find((h) => h.id === 'g17');
  const rad = (d) => (d * Math.PI) / 180;
  const g = createGameState(def, {});
  assert.ok(def.height > 2 * def.view.h && def.height > def.width, 'a hole that goes down, not across');
  assert.equal(g.maxSpeed, 440, 'the heavy charge');
  const [gold, rose] = g.wormholes;
  const cage = g.movers.find((m) => m.kind === 'orbiter');
  assert.ok(cage && Math.hypot(cage.x - rose.ax, cage.y - rose.ay) < 1, 'the second mouth sits in a turning cage');
  // The floors are sealed: every sink, at any moment, goes through both pairs.
  let sinks = 0;
  for (let T = 0; T < 5; T += 1.25) for (let d = -180; d < 180; d += 2) {
    const r = await golfFly(def, rad(d), { launchAt: T });
    if (r.end !== 'cup') continue;
    sinks++;
    assert.equal(r.warps, 2, `${d} degrees at ${T} s went down through ${r.warps} mouths`);
  }
  assert.ok(sinks > 0);
  // The line into the gold mouth goes down when the cage is open and not when it is shut.
  const open = await golfFly(def, rad(14), { launchAt: 2.0 });
  assert.equal(open.end, 'cup', `14 degrees at 2.0 s goes down (${open.end})`);
  assert.equal(open.warps, 2);
  assert.notEqual((await golfFly(def, rad(14), { launchAt: 0.75 })).end, 'cup', 'the same line when the cage is shut');
  assert.notEqual((await golfFly(def, def.tee.angle)).end, 'cup');
});

test('golf: Grand Tour - the gate, the saddle, the mouth onto an orbit, and one burn past a maw on a rail into a caged cup', async () => {
  const { COURSE } = await import('../src/golf.js');
  const def = COURSE.find((h) => h.id === 'g18');
  const rad = (d) => (d * Math.PI) / 180;
  // The gate: the same line reaches the mouth only in the moments the bar in the gate lets it through.
  const through = [];
  for (let T = 0; T < 4.9; T += 0.3) through.push((await golfFly(def, rad(-1.75), { launchAt: T, maxT: 7 })).warps > 0);
  assert.ok(through.some(Boolean) && !through.every(Boolean), 'the gate opens and shuts');
  // Through the gate, the saddle and the mouth, the charge is set down on an orbit, and with no burn it stays there.
  const parked = await golfFly(def, rad(-1.75), { launchAt: 1.5 });
  assert.equal(parked.end, 'spent');
  assert.equal(parked.warps, 1);
  // One burn of three pulses, a lap after being set down, across into the cage and the cup.
  const burn = [9.725, 9.975, 10.225].map((at) => ({ at, a: rad(-3.77) }));
  const tour = await golfFly(def, rad(-1.75), { launchAt: 1.5, pulses: burn });
  assert.equal(tour.end, 'cup', `the tour goes down (${tour.end})`);
  assert.equal(tour.warps, 1);
  assert.ok(tour.t < def.flightSeconds);
  // The burn a lap later meets the maw on the rail, and two pulses are not enough.
  const lap = (2 * Math.PI * 300) / 430;
  assert.equal((await golfFly(def, rad(-1.75), { launchAt: 1.5, pulses: burn.map((p) => ({ ...p, at: p.at + lap })) })).end, 'maw', 'a lap late, the maw on the rail is there');
  assert.notEqual((await golfFly(def, rad(-1.75), { launchAt: 1.5, pulses: burn.slice(0, 2) })).end, 'cup', 'two pulses do not carry it across');
});

test('golf: the charge is held to the arena cap, so nothing a field does to it can tunnel a wall', async () => {
  const { createGameState } = await import('../src/gamestate.js');
  const { PHYSICS_DT } = await import('../src/config.js');
  const { COURSE, FAR_COURSE, GOLF } = await import('../src/golf.js');
  for (const def of COURSE.concat(FAR_COURSE)) {
    const g = createGameState(def, {});
...
test('course music: every hole on both courses has its own track, none of them is a level\'s, and each asks the engine for a room', async () => {
  const { TRACKS } = await import('../src/audio/tracks.js');
  const { COURSE, FAR_COURSE } = await import('../src/golf.js');
  const { SEQUENCE } = await import('../src/conduits.js');
  const levelTracks = new Set(SEQUENCE.map((d) => d.track));
  const seen = new Set();
  // The Far Course plays in minor keys: its own colder music, not the first course's again.
  for (const h of FAR_COURSE) assert.match(TRACKS[h.track].key, /minor/, `${h.title} plays in a minor key`);
  for (const h of COURSE.concat(FAR_COURSE)) {
...
test('golf: a one-way pair only lets go at its far mouth, and a hole that scrolls still keeps its tee and cup inside its own room', async () => {
  ...
  for (const def of COURSE.concat(FAR_COURSE).filter((h) => h.view)) {
...
test('golf, the Far Course: no hole opens on its answer, and hardly a line in the circle sinks it bare', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  for (const def of FAR_COURSE) {
    const opener = await golfFly(def, def.tee.angle);
    assert.notEqual(opener.end, 'cup', `${def.title}: the tee must not point at the answer`);
    // Every other degree, launched as the hole opens. Each hole's own test flies its route exactly.
    let sinks = 0;
    for (let deg = -180; deg < 180; deg += 2) if ((await golfFly(def, (deg * Math.PI) / 180)).end === 'cup') sinks++;
    // The outer six: at most about one line in a hundred; the middle six, one in two hundred; the inner six, none: the gauge is needed.
    const cap = def.noBareLine ? 0 : def.hole <= 6 ? 2 : 1;
    assert.ok(sinks <= cap, `${def.title}: ${sinks} of 180 lines sink it bare`);
  }
});

test('golf, the Far Course\'s pieces: glass only a fast charge breaks, a switch that holds its door a while, stones that come and go, and emitters on the hole\'s clock', async () => {
  const { createGameState, tickOrbits, tickEmitters, paneBreaks, breakPane, golfSwitch, golfRestore, wellsAccel, solidPolysNow } = await import('../src/gamestate.js');
  const { FAR_COURSE } = await import('../src/golf.js');
  const { PHYSICS_DT } = await import('../src/config.js');
  const byId = (id) => FAR_COURSE.find((h) => h.id === id);
  const run = (g, seconds) => {
    for (let k = 0; k < Math.round(seconds / PHYSICS_DT); k++) {
      tickOrbits(g, PHYSICS_DT);
      tickEmitters(g, PHYSICS_DT);
    }
  };
  // Glass: a wall under its break speed, gone at it, and whole again at the next tee.
  const glass = createGameState(byId('f3'), {});
  const pane = glass.panes[0];
  const seg = pane.segs[0];
  const speed = glass.def.glass.breakSpeed;
  assert.equal(paneBreaks(glass, seg, { vx: speed - 1, vy: 0 }), false, 'just under the break speed, glass is a wall');
  assert.equal(paneBreaks(glass, seg, { vx: 0, vy: -speed }), true, 'at it, glass gives');
  breakPane(glass, pane, { vx: 0, vy: -speed }, glass.mouthTime);
  assert.ok(!glass.walls.includes(seg), 'a broken pane leaves the walls');
  assert.ok(Math.abs(Math.hypot(glass.ball.vx, glass.ball.vy) - speed * glass.def.glass.speedKeep) < 1e-6, 'and the charge goes on through, a little slower');
  golfRestore(glass);
  assert.ok(glass.walls.includes(seg) && !pane.broken, 'every pane is back for the next launch');
  // A switch opens its door, and it shuts again when the time is up, but never on the charge.
  const sw = createGameState(byId('f4'), {});
  const door = sw.doors[0];
  const node = sw.nodes[0];
  assert.ok(door.closed && sw.walls.includes(door.segs[0]), 'the door starts shut');
  golfSwitch(sw, node, sw.mouthTime);
  assert.ok(!door.closed && !sw.walls.includes(door.segs[0]) && node.lit, 'the switch opens it');
  sw.ball.held = true; // on the tee, nowhere near the door
  run(sw, node.holdOpen - 0.1);
  assert.ok(!door.closed, 'still open just before its time');
  run(sw, 0.2);
  assert.ok(door.closed && !node.lit, `shut again ${node.holdOpen} s after the switch`);
  golfSwitch(sw, node, sw.mouthTime);
  const mid = door.poly.reduce((a, q) => [a[0] + q[0] / door.poly.length, a[1] + q[1] / door.poly.length], [0, 0]);
  Object.assign(sw.ball, { held: false, x: mid[0], y: mid[1] });
  run(sw, node.holdOpen + 1);
  assert.ok(!door.closed, 'a door never shuts on the charge in its doorway');
  sw.ball.x -= 200;
  run(sw, 0.1);
  assert.ok(door.closed, 'and shuts as soon as it is through');
  golfSwitch(sw, node, sw.mouthTime);
  golfRestore(sw);
  assert.ok(door.closed && !node.lit, 'a new launch finds the door shut and the switch dark');
  // A stone that phases: there, pulling and solid, then gone, with neither.
  const ecl = createGameState(byId('f5'), {});
  const stone = ecl.wells.find((w) => w.phasing);
  const near = { x: stone.x + stone.r + 30, y: stone.y };
  const mover = ecl.movers.find((m) => m.well === stone);
  assert.ok(mover, 'a solid phasing stone is a mover, so its wall can come and go');
  assert.ok(!stone.absent && wellsAccel([stone], near.x, near.y) && mover.segments().length && solidPolysNow(ecl).includes(mover.polygon()), 'there at the start');
  run(ecl, stone.phasing.on + 0.5);
  assert.ok(stone.absent, `gone after its ${stone.phasing.on} s`);
  assert.equal(wellsAccel([stone], near.x, near.y), null, 'gone, it pulls nothing');
  assert.equal(mover.segments().length, 0, 'and stops nothing');
  run(ecl, stone.phasing.off);
  assert.ok(!stone.absent, `and back after its ${stone.phasing.off} s off`);
  // Emitters keep the hole's clock, so a launch at the same moment meets the same rings.
  const brk = createGameState(byId('f6'), {});
  const e = brk.emitters[0];
  run(brk, e.delay + 0.1);
  assert.ok(e.pulser.ring(), 'a ring is out just after the emitter\'s delay');
  assert.ok(Math.abs(e.pulser.t - brk.mouthTime) < 1e-9, 'on the hole\'s own clock');
  const again = [await golfFly(byId('f6'), 0.1, { launchAt: 0.1 }), await golfFly(byId('f6'), 0.1, { launchAt: 0.1 })];
  assert.deepEqual([again[0].end, again[0].t], [again[1].end, again[1].t], 'the same launch at the same moment is the same flight');
});

test('golf, the Far Course - Needle: a dry gauge, one channel a charge and a hair wide, and a line about a degree across', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f1');
  const rad = (d) => (d * Math.PI) / 180;
  assert.equal(def.fuel, 0, 'no gauge: the launch is the whole shot');
  const [upper, lower] = def.obstacles.map(obstaclePoly);
  const gap = Math.min(...lower.map((p) => p[1])) - Math.max(...upper.map((p) => p[1]));
  assert.ok(gap > 2 * BALL.radius && gap < 3 * 2 * BALL.radius, `the channel is ${gap} px: wider than the charge, narrower than three`);
  for (const d of [-45.15, -44.9, -44.65]) assert.equal((await golfFly(def, rad(d))).end, 'cup', `${d}° threads it`);
  for (const d of [-46, -43.8]) assert.notEqual((await golfFly(def, rad(d))).end, 'cup', `${d}° does not`);
});

test('golf, the Far Course - Carousel: the cup rides a rail with a maw half a turn behind, and the line through the gate needs the moment too', async () => {
  const { createGameState, placeRails } = await import('../src/gamestate.js');
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f2');
  const rad = (d) => (d * Math.PI) / 180;
  const g = createGameState(def, {});
  const cup = g.wells.find((w) => w.cup);
  const rider = g.wells.find((w) => w.hazard && w.rail);
  assert.ok(cup.rail && rider, 'the cup is on a rail, and a maw rides it too');
  const seen = [];
  for (const t of [0, 1.7, 4.5, 6.2]) {
    placeRails(g.wells, t);
    seen.push([cup.x, cup.y]);
    assert.ok(Math.abs(Math.hypot(cup.x - rider.x, cup.y - rider.y) - 2 * cup.rail.R) < 1e-6, `at ${t} s the maw is half a turn from the cup`);
  }
  assert.ok(new Set(seen.map((p) => p.map(Math.round).join())).size === seen.length, 'and the cup does not wait');
  assert.equal((await golfFly(def, rad(19), { launchAt: 0.4 })).end, 'cup', 'through the gate as the cup comes round');
  assert.notEqual((await golfFly(def, rad(19), { launchAt: 4.9 })).end, 'cup', 'the same line half a turn later finds it gone');
  // Near misses either side: one pulse aimed at the cup brings each home.
  for (const [da, at] of [[-0.5, 2.25], [-0.25, 3.25], [0.25, 3.5], [0.5, 2.75]]) {
    assert.notEqual((await golfFly(def, rad(19 + da), { launchAt: 0.4 })).end, 'cup', `${da}° misses bare`);
    assert.equal((await golfFly(def, rad(19 + da), { launchAt: 0.4, pulses: [{ at, toward: 'cup' }] })).end, 'cup', `${da}° is steered home`);
  }
});

test('golf, the Far Course - Glasshouse: only the dive past the maw breaks the floor, and a burn above it sets the charge down', async () => {
  const { FAR_COURSE, GOLF } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f3');
  const rad = (d) => (d * Math.PI) / 180;
  assert.ok(def.ball.speed + def.fuel * GOLF.pulse < def.glass.breakSpeed, 'the launch and the whole gauge together cannot break the glass: the maw has to');
  const floor = Math.min(...def.obstacles.filter((o) => o.glass).map((o) => Math.min(...o.poly.map((p) => p[1]))));
  const through = await golfFly(def, rad(-36), { maxT: 1.6 });
  assert.ok(through.y < floor, `past the maw's edge and up through the floor (${Math.round(through.y)} above ${Math.round(floor)})`);
  assert.notEqual((await golfFly(def, rad(-36))).end, 'cup', 'bare, it rises past the cup and falls back');
  const burn = [{ at: 1.37, a: rad(34) }, { at: 1.49, a: rad(42.7) }];
  for (const d of [-36.25, -36, -35.75]) assert.equal((await golfFly(def, rad(d), { pulses: burn })).end, 'cup', `${d}°, and two pulses above the glass`);
});

test('golf, the Far Course - Switchback: the switch opens the cup\'s door for four seconds, and the way back is short enough', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f4');
  const rad = (d) => (d * Math.PI) / 180;
  assert.equal(def.nodes[0].holdOpen, 4);
  const home = [{ at: 2.73, a: rad(-176.3) }];
  assert.equal((await golfFly(def, rad(-31), { pulses: home })).end, 'cup', 'off the switch and back through the door');
  assert.notEqual((await golfFly({ ...def, nodes: [] }, rad(-31), { pulses: home })).end, 'cup', 'with no switch, the door never opens');
  assert.notEqual((await golfFly(def, rad(-31))).end, 'cup', 'and it takes the gauge to come back in time');
});

test('golf, the Far Course - Eclipse: two stones on their own clocks, and a line that is only there when both are right', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f5');
  const rad = (d) => (d * Math.PI) / 180;
  const clocks = def.wells.filter((w) => w.phasing).map((w) => w.phasing.on + w.phasing.off);
  assert.equal(clocks.length, 2, 'two stones that phase');
  assert.notEqual(clocks[0], clocks[1], 'on different clocks');
  assert.equal((await golfFly(def, rad(8), { launchAt: 4 })).end, 'cup', 'launched when the clocks are right');
  assert.notEqual((await golfFly(def, rad(8), { launchAt: 2 })).end, 'cup', 'the same line two seconds earlier');
  const wells = def.wells.map((w) => ({ ...w, phasing: undefined }));
  const still = { ...def, wells, cup: wells[wells.length - 1] };
  assert.notEqual((await golfFly(still, rad(8), { launchAt: 4 })).end, 'cup', 'with both stones standing for good, the line is not there');
});

test('golf, the Far Course - Breakers: a ring that catches the charge from behind throws it past the maw; one met head on throws it back', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const { SURFACE_VELOCITY_FACTOR } = await import('../src/config.js');
  const def = FAR_COURSE.find((h) => h.id === 'f6');
  const rad = (d) => (d * Math.PI) / 180;
  const ring = def.emitters[0];
  assert.ok(ring.speed * SURFACE_VELOCITY_FACTOR.toward > def.ball.speed, 'a ring runs faster than the charge, so it can catch it from behind');
  const ride = await golfFly(def, rad(5.9), { launchAt: 0.1 });
...
test('golf, the Far Course\'s figure-eight: three bodies on one curve, crossing at its heart, never meeting', async () => {
  const { orbitPoint, railPath, createGameState, tickOrbits } = await import('../src/gamestate.js');
  const { PHYSICS_DT } = await import('../src/config.js');
  const { FAR_COURSE } = await import('../src/golf.js');
  const eight = { shape: 'eight', cx: 0, cy: 0, R: 100, period: 12, phase: 0 };
  let w = 0;
  let h = 0;
  for (const p of railPath(eight, 720)) {
    w = Math.max(w, Math.abs(p.x));
    h = Math.max(h, Math.abs(p.y));
  }
  assert.ok(Math.abs(w - 108.1) < 0.2 && Math.abs(h - 35.8) < 0.2, `2.16 of its scale across and 0.72 tall (${w.toFixed(1)} by ${h.toFixed(1)})`);
  for (const t of [0, 6]) {
    const p = orbitPoint(eight, t);
    assert.ok(Math.hypot(p.x, p.y) < 1e-9, `it crosses itself at its centre (t ${t})`);
  }
  // The dance on the hole: three maws a third of a turn apart, and in twelve seconds no two come near each other.
  const def = FAR_COURSE.find((x) => x.id === 'f16');
  const g = createGameState(def, {});
  const trio = g.wells.filter((x) => x.rail && x.rail.shape === 'eight');
  assert.equal(trio.length, 3);
  let closest = Infinity;
  for (let t = 0; t < 12; t += PHYSICS_DT) {
    tickOrbits(g, PHYSICS_DT);
    for (let i = 0; i < 3; i++) for (let j = i + 1; j < 3; j++) closest = Math.min(closest, Math.hypot(trio[i].x - trio[j].x, trio[i].y - trio[j].y));
  }
  assert.ok(closest > 150, `the three never meet (closest ${Math.round(closest)} px)`);
});

test('golf, the Far Course - Slingshot Ladder: four rungs turn a slow charge a quarter turn each, and a fast one far less', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f13');
  const rad = (d) => (d * Math.PI) / 180;
  assert.equal(def.wells.filter((w) => w.hazard).length, 4, 'four rungs');
  const back = [{ at: 1, a: rad(183) }];
  for (const d of [2.75, 3, 3.25]) assert.equal((await golfFly(def, rad(d), { pulses: back })).end, 'cup', `${d}°, and straight back on the run-up: round all four and home`);
  assert.notEqual((await golfFly(def, rad(3))).end, 'cup', 'at launch speed the first rung throws the line away');
  assert.notEqual((await golfFly(def, rad(3), { pulses: [{ at: 2, a: rad(183) }] })).end, 'cup', 'the same burn a second late is taken on the first rung, not before it');
  // The stone by the tee: no straight way to the cup behind it.
  const wells = def.wells.filter((w) => !w.solid);
  const open = { ...def, wells, cup: wells[wells.length - 1] };
  let withStone = 0;
  let without = 0;
  for (let d = 170; d <= 178; d++) {
    if ((await golfFly(def, rad(d))).end === 'cup') withStone++;
    if ((await golfFly(open, rad(d))).end === 'cup') without++;
  }
  assert.equal(withStone, 0, 'no straight line home');
  assert.ok(without > 4, `without the stone, ${without} of 9 straight lines would sink it`);
});

test('golf, the Far Course - Contraflow: two mouths circle a black hole opposite ways, beyond the reach of any launch, and only pink leads to the cup', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f14');
  const rad = (d) => (d * Math.PI) / 180;
  const [pink, gold] = def.wormholes;
  assert.ok(pink.orbitA.R === gold.orbitA.R && pink.orbitA.period === -gold.orbitA.period, 'one orbit, opposite ways');
  assert.ok(pink.oneWay && gold.oneWay);
  const hole = def.wells.find((w) => w.hazard && !w.rail && Math.hypot(w.x - pink.orbitA.cx, w.y - pink.orbitA.cy) < 1);
  assert.ok(hole, 'the body parked round is a black hole');
  // What a launch alone can climb to: nowhere near the mouths.
  let warps = 0;
  for (const T of [0, 5]) for (let d = -180; d < 180; d += 15) warps += (await golfFly(def, rad(d), { launchAt: T })).warps;
  assert.equal(warps, 0, 'no launch on its own reaches either mouth');
  const burns = [{ at: 5.14, a: rad(-27.7) }, { at: 5.39, a: rad(-12) }];
  for (const [d, T] of [[-90, 4], [-90.25, 4], [-89.75, 4], [-90, 4 - 1 / 240], [-90, 4 + 1 / 240]]) {
    const r = await golfFly(def, rad(d), { launchAt: T, pulses: burns });
    assert.equal(r.end, 'cup', `parked, and two burns into the pink mouth (${d}°, ${T} s)`);
    assert.equal(r.warps, 1);
  }
  assert.notEqual((await golfFly(def, rad(-90), { launchAt: 4, pulses: burns.map((p) => ({ ...p, at: p.at + 1 })) })).end, 'cup', 'a second late, the pink mouth has gone by');
  const trapped = await golfFly(def, rad(-90), { launchAt: 4, pulses: [{ at: 0.75, a: rad(-28.1) }, { at: 1, a: rad(-12.3) }] });
  assert.ok(trapped.warps === 1 && trapped.end === 'maw', 'the gold mouth lets go inside the trap');
});

test('golf, the Far Course - Pachinko: glass only a burned heavy charge breaks, a pulse a floor, and one line through three lenses', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f15');
  const rad = (d) => (d * Math.PI) / 180;
  assert.ok(def.glass.breakSpeed > def.ball.speed && def.glass.breakSpeed < def.maxBallSpeed, 'the glass wants more than the launch gives, and less than the cap');
  assert.ok(def.maxBallSpeed * def.glass.speedKeep < def.glass.breakSpeed, 'and a charge through one pane is too slow for the next');
  assert.equal(def.fuel, 2, 'a pulse for each floor, and none spare');
  const line = [{ at: 0.05, a: rad(104.5) }, { at: 1.68, a: rad(104.7) }];
  for (const d of [104.25, 104.5, 104.75]) assert.equal((await golfFly(def, rad(d), { pulses: line })).end, 'cup', `${d}°: through both panes`);
  assert.notEqual((await golfFly(def, rad(104.5))).end, 'cup', 'launched bare, the first pane holds');
  assert.notEqual((await golfFly(def, rad(104.5), { pulses: line.slice(0, 1) })).end, 'cup', 'with no pulse after the first pane, the second holds');
});

test('golf, the Far Course - Figure Eight: nothing the dance lets go of is fast enough for the glass, unless it burned at the bottom of a fall', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f16');
  const rad = (d) => (d * Math.PI) / 180;
  assert.ok(def.glass.breakSpeed / def.glass.speedKeep > def.maxBallSpeed / 2, 'broken into, the glass keeps too little speed to break out');
  const burns = [{ at: 0.6, a: rad(5.4) }, { at: 1.05, a: rad(12.7) }];
  for (const [d, T] of [[-2, 3], [-2.25, 3], [-1.75, 3], [-2, 3 - 1 / 240], [-2, 3 + 1 / 240]]) assert.equal((await golfFly(def, rad(d), { launchAt: T, pulses: burns })).end, 'cup', `through the dance, two burns at the bottom of the fall (${d}°, ${T} s)`);
  assert.notEqual((await golfFly(def, rad(-2), { launchAt: 3 })).end, 'cup', 'without the burns, the same dive is too slow for the glass');
  assert.notEqual((await golfFly(def, rad(-2), { launchAt: 5, pulses: burns })).end, 'cup', 'two seconds later, the dance is somewhere else');
});

test('golf, the Far Course - Long Night: park, transfer, stay, meet the mouth, and the cup at the end', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f17');
  const rad = (d) => (d * Math.PI) / 180;
  assert.equal(def.wells.length, 4, 'three bodies and the cup');
  assert.ok(def.fuel === 10 && def.flightSeconds === 45);
  const route = [[0.6, -40.4], [0.85, -24.6], [6.1, 145.1], [22.5, 6.7]].map(([at, a]) => ({ at, a: rad(a) }));
  for (const [d, T] of [[-90, 0], [-90.25, 0], [-89.75, 0], [-90, 1 / 240]]) {
    const r = await golfFly(def, rad(d), { launchAt: T, pulses: route });
    assert.equal(r.end, 'cup', `the whole night (${d}°, ${T} s)`);
    assert.equal(r.warps, 1, 'through the mouth');
  }
  assert.notEqual((await golfFly(def, rad(-90), { pulses: route.filter((p, i) => i !== 2) })).end, 'cup', 'without the burn back at the second body, it flies on past');
  assert.notEqual((await golfFly({ ...def, wormholes: [] }, rad(-90), { pulses: route })).end, 'cup', 'the third body\'s house has no way in but the mouth');
});

test('golf, the Far Course - Singularity: switch, glass, door, slot, the body that breathes, and the cup in its cage', async () => {
  const { FAR_COURSE } = await import('../src/golf.js');
  const def = FAR_COURSE.find((h) => h.id === 'f18');
  const rad = (d) => (d * Math.PI) / 180;
  assert.ok(def.nodes.length === 1 && def.doors.length === 1 && def.obstacles.some((o) => o.glass), 'a switch, a door, and glass');
  assert.ok(def.wormholes.length === 1 && def.wormholes[0].flat, 'a pair of slots');
  assert.ok(def.wells.some((w) => w.breath) && def.cup.rail && def.movers.some((m) => m.type === 'orbiter'), 'a body that breathes, a cup on a rail, and a cage');
  assert.equal(def.par, 7);
  const route = [[3.55, 15.5], [3.8, 24.6], [8.36, 263.5], [8.51, 268.8], [17, 303.8]].map(([at, a]) => ({ at, a: rad(a) }));
  for (const [d, T] of [[-61, 4.5], [-61.25, 4.5], [-60.9, 4.5], [-61, 4.5 - 1 / 240], [-61, 4.5 + 1 / 240]]) {
    const r = await golfFly(def, rad(d), { launchAt: T, pulses: route });
    assert.equal(r.end, 'cup', `the whole course in one launch (${d}°, ${T} s)`);
    assert.equal(r.warps, 1, 'down the slot');
  }
  assert.notEqual((await golfFly({ ...def, nodes: [] }, rad(-61), { launchAt: 4.5, pulses: route })).end, 'cup', 'with no switch, the door stays shut');
  assert.notEqual((await golfFly(def, rad(-61), { launchAt: 4.5, pulses: route.filter((p, i) => i !== 1) })).end, 'cup', 'with one burn as the glass goes, the maw pulls the charge back');
  assert.notEqual((await golfFly(def, rad(-61), { launchAt: 4.5, pulses: route.slice(0, 4), maxT: 25 })).end, 'cup', 'parked round the body, the cup does not come by itself in time');
});
